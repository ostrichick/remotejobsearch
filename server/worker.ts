...
async function limit(env: Env, uid: string, action: string, max: number) {
  ...
  const r = await env.DB.prepare(
    ...
    .first<{ count: number }>();
  if ((r?.count ?? 0) > max)
    throw new Error('오늘의 사용 한도에 도달했습니다. 내일 다시 시도하세요.');
}
export async function api(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (url.pathname === '/api/analyze' && request.method === 'POST') {
      ...
      if (Number(request.headers.get('content-length') ?? 0) > effectiveMaxBodyBytes(env))
        return json({ error: '최대 파일 크기는 5MB입니다.' }, 413);
    ...
    if (url.pathname === '/api/data' && request.method === 'DELETE') {
      ...
      if (row) await env.FILES.delete(row.object_key);
      await env.DB.batch(
        ['profiles', 'resumes', 'searches', 'saved'].map((t) =>
    ...
    if (url.pathname === '/api/search' && request.method === 'GET')
      return json(await getData(env, 'searches', uid));
    if (url.pathname === '/api/search' && request.method === 'POST') {
      const profile = await getData(env, 'profiles', uid);
      ...
      if (
        ...
        JSON.stringify(previous.keywords) === JSON.stringify(profile.keywords) &&
        JSON.stringify(previous.preferences) === JSON.stringify(profile.preferences)
      )
      ...
      await env.DB.prepare(
        'INSERT INTO searches(user_id,data,updated_at) VALUES(?,?,?) ON CONFLICT(user_id) DO UPDATE SET data=excluded.data,updated_at=excluded.updated_at',
      )
        .bind(uid, JSON.stringify(result), result.searchedAt)
        .run();
      return json(result);
    }
    ...
    return json(
      {
        error:
          e instanceof Error && /프로필|한도|키워드|길이|형식/.test(e.message)
            ? e.message
...
async function readJson(request: Request) {
  ...
  if (text.length > 100000) throw new Error('요청 길이 초과');
